...
+(CCScene *) scene
{
	...
	ECardLayer *layer = [ECardLayer node];
...
- (id) init {
	if((self = [super init])) {
        ...
        self.isTouchEnabled = YES;  //터치가능하게함
        doTouch = YES;
        ...
        cardData = [imageDataSet.cardSet objectAtIndex:randNum];
        imageBg = [CCSprite spriteWithSpriteFrame:[cardData objectAtIndex:delegate.eng_num]];
        [imageBg setAnchorPoint:CGPointZero];
        [imageBg setPosition:CGPointZero];
		[self addChild:imageBg z:0];

        imageData = [imageDataSet.imageSet objectAtIndex:randNum];
        image = [CCSprite spriteWithSpriteFrame:[imageData objectAtIndex:delegate.eng_num]];
...
-(BOOL)ccTouchBegan:(UITouch *)touch withEvent:(UIEvent *)event
{
    ...
    if(ccpDistance(image.position, convertedLocation) < image.contentSize.width /2) {
        ...
        eSoundDataSet = [ImageSetData sharedData];
        eSoundData = [eSoundDataSet.eSoundSet objectAtIndex:randNum];
        
        sae = [SimpleAudioEngine sharedEngine];
        [sae playEffect:eSoundData[delegate.eng_num]];
        ...
        id jump2 = [CCScaleTo actionWithDuration:3 scale:1];
        id jump3 = [CCJumpTo actionWithDuration:3 position:ccp(160,150) height:200 jumps:3];
        ...
        id rotate4_3 = [CCRotateTo actionWithDuration:1.5 angle:360*3];
